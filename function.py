...
def wash(dry=False, water=8):     # 寫參數的時候，等號左右不用空格  # 參數可以有"預設值"
    print('加水', water, '分滿')
    print('加洗衣精')
    print('旋轉')
    if dry:
...
# Return(回傳)就像自動販賣機投進去後會吐出飲料
# function如果有return，就可以把funciton執行的結果給存下來。
# 可以想像為function運行完後會化身成為return的東西。

def add(x, y):          # 投東西的時候是自動按照參數的順序
    return(x + y)

result = add(3, 4)      # 可以把function的執行結果存下來
print(result)

#寫一個算出清單平均的function
def average(numbers):                   # 設立一個清單numbers作為參數                    
    return(sum(numbers) / len(numbers)) # 清單算出平均以後 # 就可以return回傳出來
print(average([1, 2, 3]))               # 傳一個清單[1, 2, 3]進去